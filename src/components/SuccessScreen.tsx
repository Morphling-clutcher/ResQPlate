...
import { X, Check, Share2, Globe, Sparkles, Copy, Twitter, MessageSquare, Landmark, ShoppingBag } from 'lucide-react';
...
export default function SuccessScreen({
  ...
  return (
    <div className="min-h-screen bg-brand-bg flex flex-col max-w-md mx-auto relative pb-6" id="success-screen-container">
      
      {/* Header bar */}
      <div className="px-4 py-4 flex items-center justify-between bg-transparent border-b border-slate-100" id="success-header">
      ...
      <div className="p-4 space-y-5 flex-1 overflow-y-auto" id="success-scroll-content">
        
        {/* Animated Double-Ring Checkmark Block */}
        <div className="flex flex-col items-center text-center py-4" id="success-celebration-block">
          <motion.div 
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ type: 'spring', damping: 15 }}
            className="w-20 h-20 rounded-full bg-emerald-50 flex items-center justify-center relative border-4 border-emerald-100 shadow-md mb-4"
            id="checkmark-outer-ring"
          >
            <div className="w-14 h-14 rounded-full bg-emerald-500 flex items-center justify-center text-white" id="checkmark-inner-ring">
              <Check className="w-7 h-7 stroke-[3]" />
            </div>
            {/* Subtle floating leaf element */}
            <div className="absolute -top-1 -right-1 bg-brand-primary text-white p-1 rounded-full text-[9px]">
              🌱
            </div>
          </motion.div>

          <h3 className="text-3xl font-extrabold text-brand-text tracking-tight" id="success-heading">
            Enjoy your meal!
          </h3>
          <p className="text-sm text-brand-text-muted mt-2 px-6" id="success-subheading">
            Your pickup at <strong className="text-brand-text font-bold">{store.name}</strong> is complete.
          </p>
        </div>

        {/* Card 1: Order Details */}
        <div className="bg-white p-5 rounded-2xl border border-brand-outline shadow-level-2" id="success-order-details-card">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3 text-xs" id="success-order-header-row">
            <div>
            ...
            <div className="text-right">
              <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">DATE & TIME</span>
              <p className="font-semibold text-brand-text mt-0.5" id="success-order-timestamp">{formattedDate}, {formattedTime}</p>
            </div>
          </div>

          {/* Purchased item itemizer row */}
          <div className="flex items-center gap-3.5 pt-4" id="success-order-item-row">
            ...
            <div className="flex-1 min-w-0" id="success-order-item-meta">
              <h4 className="text-sm font-extrabold text-brand-text truncate">{bag.name}</h4>
              <p className="text-[11px] text-brand-text-muted mt-0.5 font-medium">Quantity: {quantity} • Paid via {paymentMethod === 'GCash' ? 'GCash' : 'Cash on Pickup'}</p>
            </div>
            <div className="text-right shrink-0" id="success-order-item-price">
              <span className="text-sm font-black text-brand-primary font-mono">₱{totalDiscountedPrice.toFixed(2)}</span>
            </div>
          </div>
        </div>

        {/* Card 2: Impact Unlocked Card (Emerald green background as specified) */}
        <div className="bg-emerald-600 text-white p-5 rounded-2xl shadow-level-2 relative overflow-hidden" id="success-impact-unlocked-card">
          {/* Subtle overlay green vector leaf shape */}
          ...
          {/* Card Top Title with Star icon */}
          <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-emerald-100" id="impact-unlocked-label-row">
            <Sparkles className="w-4 h-4 text-amber-300 fill-amber-300" />
            <span>Impact Unlocked</span>
          </div>

          {/* Large dynamic Saved weight Display */}
          <div className="mt-2.5 pb-3 border-b border-emerald-500/40" id="impact-unlocked-weight-row">
            <h4 className="text-4xl font-extrabold tracking-tight font-sans" id="success-today-impact">
              {currentSavedGrams}g
            </h4>
            <p className="text-[11px] text-emerald-100 mt-0.5 font-medium">saved from waste today</p>
          </div>

          {/* Lifetime Impact Footer */}
          <div className="flex items-center justify-between pt-3" id="lifetime-impact-row">
            <div>
              <h5 className="text-lg font-bold font-sans" id="success-lifetime-impact">
                {newLifetimeKg.toFixed(1)}kg
              </h5>
              <p className="text-[10px] text-emerald-200 uppercase tracking-wider font-semibold">Lifetime Impact</p>
            </div>

            {/* Micro badges icons */}
            <div className="flex gap-1.5" id="impact-globe-badge">
              <div className="w-8 h-8 rounded-full bg-emerald-500/50 flex items-center justify-center border border-emerald-400/30">
        ...
        <div className="text-center space-y-2.5 py-2" id="inspire-community-section">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-brand-text text-xs font-bold rounded-full transition-all cursor-pointer active:scale-95"
            id="btn-share-impact"
          >
      ...
      {/* Persistent Bottom Discovery Return Action */}
      <div className="p-4" id="success-bottom-bar">
        <button
          onClick={onBackToDiscovery}
      ...
      {/* Share Dialog Sheet (Interactivity overlay) */}
      <AnimatePresence>
        {showShareModal && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center p-4" id="share-modal-overlay">
            <motion.div 
              initial={{ y: 200, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              exit={{ y: 200, opacity: 0 }}
              className="bg-brand-card w-full max-w-sm rounded-t-3xl p-5 space-y-4 shadow-level-3 relative"
              id="share-modal"
            >
              <div className="flex items-center justify-between" id="share-modal-header">
                <h4 className="text-base font-extrabold text-brand-text">Spread the Word!</h4>
                <button 
              ...
              <div className="grid grid-cols-3 gap-2 py-2" id="share-channels-grid">
                ...
                >
                  ...
                  <span className="text-[11px] font-bold text-slate-600">Twitter/X</span>
                </button>
                ...
                >
                  ...
                  <span className="text-[11px] font-bold text-slate-600">Chat</span>
                </button>
              </div>
